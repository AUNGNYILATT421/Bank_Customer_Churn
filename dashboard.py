...
def render_filter(df,filter_columns,filter_layout = None):
    ...
    for col,selected_value in filter_values.items():
        ...
        if selected_value != 'all':
            filtered_df = filtered_df[filtered_df[col]==selected_value].copy()
    return filtered_df
...
if page == "Churn Analysis":
    ...
    print(f"strong_engagement_customers: {strong_engagement_customers}")
    ...
    strong_engagement_churn_rate_in_churn_customers = f'{round(len(strong_engagement_churn_customers) / (len(churn_customers)+1e-9)*100,2)}%' # to fix division zero error 
    strong_engagement_churn_rate_in_engagment_customers = f'{round(len(strong_engagement_churn_customers) / (len(strong_engagement_customers)+1e-9)*100,2)}%' # to fix division zero error
...
if page == "Churn Prediction":
    ...
    with visual_pane:
        ...
        kpi1.metric('Avg Churn Probability',f"{round(filtered_df['churn_proba'].mean()*100,2)}%")
        kpi2.metric('Avg Stay Probability',f"{round(filtered_df['no_churn_proba'].mean()*100,2)}%")
...
""")
    ...
    potential_high_risk_customers = filtered_df[(filtered_df['churn_proba']>=0.7) & (filtered_df['exited'] == 0)].copy()
...
if page == "Potential Active Customers":
    ...
    with left_section:
        ...
        potential_table = filtered_potential_df[['customerid','surname','geography',
       'gender', 'age',   'numofproducts', 'tenure','balance',
        'creditscore', 'estimatedsalary']].copy()
